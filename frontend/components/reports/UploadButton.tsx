...
export function UploadButton({ profileId, variant = 'icon', label }: UploadButtonProps) {
  ...
  const modal = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[440px] bg-white rounded-3xl p-6 shadow-xl">
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-display text-xl font-semibold">Upload Report</h3>
          <button onClick={close} className="p-2 rounded-xl hover:bg-muted transition-colors">
            <X size={20} />
          </button>
        </div>
