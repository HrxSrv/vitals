        ...
        <div className="animate-fade-up animate-delay-100 mb-6">
          <button
            ...
            onClick={async () => {
              ...
              setServerError('');
              try { await signInWithGoogle(); } catch { setServerError('Google sign-up failed.'); setGoogleLoading(false); }
            }}
            className="w-full py-3.5 rounded-2xl border border-border bg-white hover:bg-muted text-foreground font-semibold text-sm flex items-center justify-center gap-3 transition-colors disabled:opacity-60"
          >
            {googleLoading ? (
